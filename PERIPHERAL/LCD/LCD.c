...
#define CURVEL_LONG 240

/***画曲线***/
//color :颜色
//cu    :数据
void LCD_DrawCurve(u16 x1, u16 y1, u16 x2, u16 y2, u8 dir, u8 num, u16 color, s16 cu)
{
    u32 index = 0;
    static s16 curve[3][CURVEL_LONG] = {0};//存放曲线
    curve[num][x2 - x1 - 1] = cu;//将当前数值放入曲线末尾
    for (index = 0; index < x2 - x1 - 1; index++)
        curve[num][index] = curve[num][index + 1];//将曲线前移一个
    for (index = 1; index < x2 - x1  + 1; index++)
    {
        if (curve[num][index - 1] != 0)
        {
            LCD_WR_CMD(lcddev.setycmd, y2 - curve[num][index - 1]);
            LCD_WR_CMD(lcddev.setxcmd, x1 + index);
            LCD_WR_REG(lcddev.wramcmd);                              //准备写数据显示区
            LCD->LCD_RAM = WHITE;//清除上次点

            LCD_WR_CMD(lcddev.setycmd, y2 - curve[num][index]);
            LCD_WR_CMD(lcddev.setxcmd, x1 + index);
            LCD_WR_REG(lcddev.wramcmd);                              //准备写数据显示区
            LCD->LCD_RAM = color;//写入 当前点
        }
    }
}
void LCD_DrawFixedCurve(u16 x1, u16 y1, u16 x2, u16 y2, u8 dir, u8 num, u16 color, s8 *p)
{
    u32 index = 0;
    for (index = 1; index < x2 - x1  + 1; index++)
    {
        if (p[index - 1] != 0)
        {
            LCD_WR_CMD(lcddev.setycmd, y2 - p[index] * 2);
            LCD_WR_CMD(lcddev.setxcmd, x1 + index);
            LCD_WR_REG(lcddev.wramcmd);                              //准备写数据显示区
            LCD->LCD_RAM = color;//写入 当前点
        }
...
void LCD_DrawPointCurve(u16 x1, u16 y1, u16 x2, u16 y2, u8 dir, u8 num, u16 color, s16 y, s16 x)
{

    LCD_WR_CMD(lcddev.setycmd, y2 - y);
    LCD_WR_CMD(lcddev.setxcmd, x1 + x);
    LCD_WR_REG(lcddev.wramcmd);                              //准备写数据显示区
    LCD->LCD_RAM = color;//写入 当前点
